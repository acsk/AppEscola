...
const toEndpointVariants = (base) => {
  ...
  if (/\/version\/mobile$/i.test(cleaned)) {
    return [cleaned];
  }

  const variants = [`${cleaned}/version/mobile`];
  if (!/\/api$/i.test(cleaned)) {
    variants.push(`${cleaned}/api/version/mobile`);
  }
...
const buildPublishTargets = () => {
  ...
  // Permite sobrescrever com múltiplas bases:
  // APP_VERSION_API_URLS="http://localhost:4000/api,https://api.appcurso.com.br/api"
  if (rawList.trim()) {
    return rawList
      .split(',')
      .map((item) => item.trim())
...
const run = async () => {
  ...
  if (targets.length === 0) {
    console.warn('⚠️ Publicação da versão no backend ignorada: endpoint inválido.');
    return;
  }
