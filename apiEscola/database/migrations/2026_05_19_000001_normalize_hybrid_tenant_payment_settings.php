...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::table('tenant_settings')
            ...
            ->each(function ($row): void {
                ...
                if ($row->key === 'default_method' && in_array($value, ['hybrid', 'boleto'], true)) {
                    $value = 'bank_slip';
                }

                if ($row->key === 'enabled_methods' && is_array($value)) {
                    $value = array_values(array_unique(array_map(
                        static fn ($method): string => in_array((string) $method, ['hybrid', 'boleto'], true)
                            ? 'bank_slip'
                            : (string) $method,
                        $value
                    )));
    ...
    {
        // Sem rollback: "hybrid"/"boleto" eram aliases e agora são representados por "bank_slip".
    }
};
