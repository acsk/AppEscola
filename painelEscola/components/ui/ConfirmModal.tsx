...
export default function ConfirmModal({
  ...
  return (
    <OverlayPortal open={visible} onClose={onCancel} contentPadding={horizontalPadding}>
      <View
      ...
      >
        <View className="items-center mb-5">
          ...
          <Text className="text-sm text-gray-500 text-center mt-1.5">{message}</Text>
        </View>
        ...
        <View style={{ flexDirection: isMobile ? "column" : "row", gap: 12 }}>
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text className="text-sm font-semibold text-gray-700">{cancelLabel}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={onConfirm}
            disabled={loading || confirmDisabled}
            className={`flex-1 py-3 rounded-xl ${confirmBg} items-center ${
              loading || confirmDisabled ? "opacity-50" : ""
            }`}
            activeOpacity={0.8}
          >
            {loading ? (
